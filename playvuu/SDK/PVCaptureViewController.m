...
@interface PVCaptureViewController ()
{
    CGSize _portraitSize;
    BOOL isRecording;
    PVOutputFrameWriter *imageWriter;
}
...
@implementation PVCaptureViewController
...
- (void) viewDidAppear:(BOOL)animated
{
    ...
    // Bullshit. Probably only position of the frame center. We need a minimum resolution.
    imageWriter = [PVOutputFrameWriter writerWithRect:(CGRectMake(0, 0, 0.5f, 0.5f))];
    [_playvuuEngine addGPUImageTarget:imageWriter forStage:PlayvuuEngineStagePostprocessor];
}
...
- (void) setupPlayvuuEngine
{
    ...
    __unsafe_unretained PVCaptureViewController * self_ = self;
...
- (void) saveRecording
{
    [imageWriter endProcessing];
    //PVLibrary *library = [PVLibrary currentLibrary];
    
    NSString *prjName = [NSString stringWithFormat:@"Project %u", arc4random()%150];
    PVProject *currentProject = [PVProject newProjectWithTitle:prjName];
    NSData * slideData = UIImagePNGRepresentation([imageWriter getImage]);
    
    
    [currentProject saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
        if(succeeded){
            NSString *fileName = [NSString stringWithFormat:@"%@.png", currentProject.objectId];
            currentProject.slideTexture = [PFFile fileWithName:fileName data:slideData];
            
            [currentProject.slideTexture saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
                if(succeeded){
                    [currentProject saveEventually];
                    //[PVLibrary addProject:currentProject];
                    //[[PVLibrary currentLibrary] saveEventually];
                }else
                    NSLog(@"Couldn't save slideshow file: %@", error);
            }];
        }else
            NSLog(@"Couldn't save current Project: %@", error);
        
    }];
    
    [_playvuuEngine saveRecording];
    [_recordButton setImage:[UIImage imageNamed:@"camera_button.png"] forState:nil];
}
...
- (IBAction)toggleRecord:(id)sender {
    if(!isRecording){
        ...
        [self startRecording];
        [_recordButton setImage:[UIImage imageNamed:@"stop_button.png"] forState:nil];
    }else{
